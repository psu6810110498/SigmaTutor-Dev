...
// ============================================================
// Course-Specific Admin Reviews Page
// Path: /admin/reviews/[courseId]
// ============================================================
...
export default function CourseAdminReviewsPage() {
  ...
  const fetchReviews = useCallback(async (pageNum: number, sortOption: SortOption) => {
    setLoading(true);
    try {
      const res = await reviewApi.adminList({ page: pageNum, limit: 10, courseId, sort: sortOption });
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-gray-500 mt-1">
            {reviews.length > 0 && reviews[0].course ? reviews[0].course.title : 'กำลังโหลด...'}
          </p>
        </div>

        {/* Sort Dropdown */}
        <div className="flex items-center gap-3">
          <label htmlFor="sort" className="text-sm font-medium text-gray-700">เรียงตาม:</label>
          <select
            id="sort"
            value={sort}
            onChange={(e) => {
            ...
            className="px-4 py-2 bg-white border border-gray-200 rounded-lg text-sm font-medium focus:outline-none focus:ring-2 focus:ring-primary/20 hover:border-primary/50 cursor-pointer shadow-sm transition-all"
          >
          ...
          <table className="w-full text-left border-collapse">
            ...
            <tbody className="divide-y divide-gray-100">
              {loading ? (
                ...
                <tr>
                  <td colSpan={4} className="p-12 text-center text-gray-500">
                    ไม่พบข้อมูลรีวิวในระบบสำหรับคอร์สนี้
                  </td>
                </tr>
              ) : (
                reviews.map((review) => (
                  <tr key={review.id} className="hover:bg-gray-50/50 transition-colors">
                    <td className="p-4 align-top w-1/4">
                      ...
                      <div className="text-xs text-gray-500 mt-1">
                        {/* Assuming email might not be in the updated user relation from Types.ts, checking if it exists */}
                        {'email' in review.user ? (review.user as any).email : ''}
                      </div>
