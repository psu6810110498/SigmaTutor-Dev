...
export default function AdminStudentsPage() {
  ...
  const fetchStudents = async () => {
    try {
      setLoading(true);
      const res = await fetch((process.env.NEXT_PUBLIC_API_URL || 'http://localhost:4000/api') + '/users/students', {
        credentials: 'include',
      });
  ...
  const totalEnrolledCourses = students.reduce(
    (sum: number, s: any) => sum + (s.enrolledCourses?.length || 0),
    0
  );
                        ...
                        <div className="w-10 h-10 rounded-full bg-slate-100 overflow-hidden ring-2 ring-white shadow-sm shrink-0">
                          <img
                            src={
                              ...
                              `https://api.dicebear.com/9.x/avataaars/svg?seed=${student.id}`
                            }
                            alt={student.name}
          ...
          >
            ...
            <div className="p-4 border-t border-slate-100 flex items-center justify-between text-sm shrink-0">
              ...
              <span className="font-black text-blue-600">
                {coursePopup.student.enrolledCourses?.length || 0} คอร์ส
              </span>
            </div>
          </div>
        </div>
