...
import {
    ...
    ArrowUp, ArrowDown, Image as ImageIcon, RefreshCw
} from "lucide-react";
            ...
            <div className="bg-gray-50 md:bg-white rounded-xl md:shadow-sm border-0 md:border border-gray-100 overflow-hidden">
                <table className="w-full text-left border-collapse block md:table">
                    ...
                    <tbody className="block md:table-row-group space-y-4 md:space-y-0 divide-y-0 md:divide-y divide-gray-50 p-4 md:p-0">
                        {isLoading ? (
                            <tr className="block md:table-row"><td colSpan={5} className="block md:table-cell p-8 text-center text-gray-400">Loading...</td></tr>
                        ) : banners.length === 0 ? (
                            <tr className="block md:table-row"><td colSpan={5} className="block md:table-cell p-8 text-center text-gray-400">ยังไม่มี Banner</td></tr>
                        ) : (
                            banners.map((banner) => (
                                <tr key={banner.id} className={`block md:table-row bg-white rounded-2xl md:rounded-none shadow-sm md:shadow-none border border-gray-100 md:border-0 p-4 md:p-0 hover:bg-gray-50/50 transition-colors ${isTrashMode ? 'opacity-70 bg-red-50/20' : ''}`}>
                                    <td className="hidden md:table-cell p-4 text-gray-500 font-mono">{banner.priority}</td>
                                        ...
                                        <div className="flex justify-between items-start">
                                            <div className="flex items-center gap-3">
                                                ...
                                                {(() => {
                                                    ...
                                                    return isTrashMode ? (
                                                        <span className="px-2 py-0.5 rounded text-[10px] font-bold border border-red-100 bg-red-50 text-red-500">ถูกลบออกจากระบบ</span>
                                                    ) : (
                                                        <span className={`px-2 py-0.5 rounded text-[10px] font-bold border ${status.color}`}>{status.label}</span>
                                                    )
                                                })()}
                                            </div>
                                            {/* Mobile Actions */}
                                            <div className="flex items-center gap-2">
                                                {isTrashMode ? (
                                                    <>
                                                        <button onClick={() => setRestoringId(banner.id)} className="p-1.5 text-green-600 bg-green-50 rounded-lg"><RefreshCw size={16} /></button>
                                                        <button onClick={() => setPromptAction({ id: banner.id, title: banner.title })} className="p-1.5 text-red-600 bg-red-50 rounded-lg"><Trash2 size={16} /></button>
                                                    </>
                                                ) : (
                                                    <>
                                                        <button onClick={() => handleOpenEdit(banner)} className="p-1.5 text-blue-600 bg-blue-50 rounded-lg"><Edit size={16} /></button>
                                                        <button onClick={() => setDeletingId(banner.id)} className="p-1.5 text-red-600 bg-red-50 rounded-lg"><Trash2 size={16} /></button>
                                                    </>
                                                )}
                                            </div>
                                        </div>
                                    ...
                                    <td className="block md:table-cell p-0 md:p-4 mb-3 md:mb-0">
                                        <div className="w-full md:w-24 h-32 md:h-12 bg-gray-100 md:rounded-lg rounded-xl overflow-hidden relative border border-gray-200">
                                            {banner.imageUrl ? (
                                                <img src={banner.imageUrlMobile || banner.imageUrl} alt={banner.title} className="w-full h-full object-cover" />
                                            ) : (
                                    ...
                                    <td className="block md:table-cell p-0 md:p-4 mt-3 md:mt-0">
                                        ...
                                        <div className="text-xs text-gray-500 line-clamp-2 max-w-xs">{banner.subtitle || '-'}</div>
                                    </td>
                                    ...
                                    <td className="flex items-center justify-between md:table-cell p-0 md:p-4 mt-2 md:mt-0 pt-2 md:pt-0 border-t border-gray-50 md:border-0">
                                        <span className="md:hidden text-[10px] text-gray-400 font-bold uppercase tracking-wider">ตำแหน่ง</span>
