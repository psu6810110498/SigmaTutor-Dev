    ...
    <div className="bg-gray-50 rounded-xl p-4 space-y-3 border border-gray-200">
            ...
            <div className="flex gap-2">
              <input
                ...
                onChange={(e) => {
                  ...
                  set('materialUrl', e.target.value);
                }}
              ...
              {form.gumletVideoId && !videoUploading ? (
                ...
                <div className="flex gap-2">
                  <input
                    ...
                    onChange={(e) => {
                      set('gumletVideoId', e.target.value);
                      if (e.target.value) set('videoProvider', 'GUMLET');
                    }}
      ...
      <div className="flex justify-end gap-2 pt-1">
        <Button
          ...
          size="sm"
          onClick={onCancel}
          disabled={loading || videoUploading || pdfUploading}
        >
          ยกเลิก
        </Button>
        ...
        <div className="flex items-center gap-3 text-xs text-gray-400 shrink-0">
          {videoCount > 0 && (
            <span className="flex items-center gap-1">
              <Video size={12} /> {videoCount} วิดีโอ
            </span>
          )}
          {chapterDuration > 0 && (
            <span className="flex items-center gap-1">
              <Clock size={12} /> {formatDuration(chapterDuration)}
            </span>
          )}
        </div>
...
export function LessonsTab({ course, onUpdate }: LessonsTabProps) {
  ...
  const totalVideos = chapters.reduce(
    (sum, ch) => sum + (ch.lessons || []).filter((l) => l.type === 'VIDEO').length,
    0
  );
        ...
        <div>
          ...
          {(totalVideos > 0 || totalMinutes > 0) && (
            <div className="flex items-center gap-4 mt-1 text-sm text-gray-500">
              {totalVideos > 0 && (
                <span className="flex items-center gap-1">
                  <Video size={14} /> {totalVideos} วิดีโอ
                </span>
              )}
              {totalMinutes > 0 && (
                <span className="flex items-center gap-1">
                  <Clock size={14} /> รวม {formatDuration(totalMinutes)}
                </span>
              )}
            </div>
          )}
