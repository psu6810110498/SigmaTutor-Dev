...
export function OnsiteScheduleTab({ course, onUpdate }: OnsiteScheduleTabProps) {
  ...
  useEffect(() => {
    ...
    if (course.schedules && course.schedules.length > 0) {
      ...
      mapped.forEach((s, i) => {
        toggles[i] = s.gumletVideoId ? 'GUMLET' : 'YOUTUBE';
      });
      setVideoSourceToggle(toggles);
      // Initialize PDF file names
      const names: Record<number, string | null> = {};
      mapped.forEach((s, i) => {
        names[i] = s.materialUrl ? s.materialUrl.split('/').pop() || null : null;
      });
  ...
  function updateSession<K extends keyof OnsiteSession>(
    index: number,
    key: K,
    value: OnsiteSession[K]
  ) {
  ...
  function removeSession(index: number) {
    if (sessions.length === 1) {
      toast.error('ต้องมีอย่างน้อย 1 ครั้ง');
      return;
    }
    setSessions((prev) =>
      prev
        .filter((_, i) => i !== index)
        .map((s: OnsiteSession, i: number) => ({ ...s, sessionNumber: i + 1 }))
    );
  }
    ...
    await new Promise<void>((resolve) => {
      ...
      xhr.upload.onprogress = (ev) => {
        if (ev.lengthComputable)
          setPdfProgress((prev) => ({
            ...prev,
            [index]: Math.round((ev.loaded / ev.total) * 100),
          }));
      };
      xhr.onload = () => {
        try {
      ...
      <div className="flex justify-between items-center pb-5 border-b">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">
            จัดการตารางเรียนสถานที่จริง สถานะ และวิดีโอย้อนหลัง
          </p>
        </div>
        <Button
          type="button"
      ...
      <div className="space-y-4">
        {sessions.map((session, index) => (
          <div key={index} className="border border-gray-200 rounded-xl overflow-hidden">
            {/* Session header */}
            <div
              className={`flex items-center justify-between px-4 py-3 border-b ${STATUS_COLORS[session.status]}`}
            >
                ...
                {session.materialUrl && !pdfUploading[index] ? (
                  <div className="flex items-center gap-2 px-3 py-2 bg-orange-50 border border-orange-200 rounded-lg text-sm">
                    ...
                    <button
                      type="button"
                      onClick={() => {
                        updateSession(index, 'materialUrl', '');
                        updateSession(index, 'content', '');
                        setPdfFileName((prev) => ({ ...prev, [index]: null }));
                      }}
                  ...
                  <div className="flex gap-2">
                    <input
                      ...
                      onChange={(e) => {
                        ...
                        updateSession(index, 'materialUrl', e.target.value);
                      }}
                ...
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex bg-gray-100 rounded-lg p-0.5">
                    <button
                      type="button"
                      onClick={() =>
                        setVideoSourceToggle((prev) => ({ ...prev, [index]: 'YOUTUBE' }))
                      }
                    ...
                    <button
                      type="button"
                      onClick={() =>
                        setVideoSourceToggle((prev) => ({ ...prev, [index]: 'GUMLET' }))
                      }
                      className={`px-3 py-1 text-xs font-semibold rounded-md transition-all duration-200 ${
