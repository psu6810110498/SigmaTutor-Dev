...
import {
    ArrowLeft, BookOpen, Clock, Users, PlayCircle, Star,
    MapPin, Calendar, Video, ShoppingCart, ChevronDown, ChevronUp,
    ThumbsUp, ExternalLink, Loader2
} from "lucide-react";
...
export default function CourseDetailPage() {
    ...
    if (!course) {
        return (
            <div className="flex flex-col items-center justify-center min-h-[60vh] text-center p-8">
                ...
                <Link href="/explore" className="text-primary font-bold hover:underline">← กลับหน้าสำรวจ</Link>
            </div>
        );
    }
    ...
    return (
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
            {/* Back */}
            <Link href="/explore" className="inline-flex items-center gap-2 text-sm text-gray-500 hover:text-primary mb-6 transition-colors">
            ...
            <div className="flex flex-col lg:flex-row gap-8">
                ...
                <div className="flex-1 min-w-0">
                    ...
                    <div className="rounded-2xl overflow-hidden bg-gray-100 h-64 md:h-80 relative mb-6">
                        ...
                        {/* Type Badge */}
                        <span className={`absolute top-4 left-4 px-3 py-1.5 rounded-lg text-sm font-medium border ${typeBadge[course.courseType].className}`}>
                    ...
                    <div className="flex flex-wrap items-center gap-4 mb-6 text-sm text-gray-500">
                        ...
                        {course.category && <span className="px-2 py-0.5 bg-gray-100 rounded-full text-xs">{course.category.name}</span>}
                        {course.level && <span className="px-2 py-0.5 bg-gray-100 rounded-full text-xs">{course.level.name}</span>}
                    </div>
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
                        {course.courseType === "ONLINE" && (
                            <>
                                <StatCard icon={<PlayCircle size={20} />} value={`${course.videoCount}`} label="วิดีโอ" />
                                <StatCard icon={<Clock size={20} />} value={course.duration || "-"} label="ระยะเวลา" />
                            </>
                        )}
                        {course.courseType === "ONLINE_LIVE" && (
                            <>
                                <StatCard icon={<Users size={20} />} value={`${course.maxSeats || "-"}`} label="จำนวนรับ" />
                                <StatCard icon={<Video size={20} />} value="Zoom" label="ช่องทาง" />
                            </>
                        )}
                        {course.courseType === "ONSITE" && (
                            <>
                                <StatCard icon={<MapPin size={20} />} value={course.location?.slice(0, 20) || "-"} label="สถานที่" />
                                <StatCard icon={<Users size={20} />} value={`${course.maxSeats || "-"}`} label="จำนวนรับ" />
                            </>
                        )}
                        <StatCard icon={<Users size={20} />} value={`${course._count?.enrollments || 0}`} label="ผู้เรียน" />
                        <StatCard icon={<Star size={20} />} value={reviewStats ? reviewStats.average.toFixed(1) : "-"} label="คะแนน" />
                    </div>
                        ...
                        <div className="bg-gradient-to-br from-primary/5 to-blue-50 rounded-2xl p-6 border border-primary/10">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                                {[
                                    ...
                                    { step: "3", title: "รับลิงก์", desc: course.courseType === "ONLINE_LIVE" ? "รับลิงก์ Zoom ทางอีเมล" : "รับรายละเอียดสถานที่เรียน" },
                    ...
                    <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6 sticky top-28 space-y-5">
                        ...
                        <div>
                            {course.originalPrice && course.originalPrice > course.price && (
                                <span className="text-sm text-gray-400 line-through block">฿{formatPrice(course.originalPrice)}</span>
                            )}
                        ...
                        <div className="space-y-3 text-sm">
                            {course.maxSeats && (
                                <div className="flex items-center justify-between text-gray-600">
                                    <span>จำนวนรับ</span>
                                    <span className="font-medium">{course._count?.enrollments || 0}/{course.maxSeats}</span>
                                </div>
                            )}
                            {course.duration && (
                                <div className="flex items-center justify-between text-gray-600">
                        ...
                        <div className="flex items-center justify-around pt-4 border-t border-gray-100 text-center">
                            ...
                            <div>
                                <p className="text-lg font-bold text-gray-900">{reviewStats ? reviewStats.average.toFixed(1) : "-"}</p>
