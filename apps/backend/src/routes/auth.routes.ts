...
router.post('/logout', async (req: Request, res: Response): Promise<void> => {
  ...
  } catch { res.json({ success: true, message: 'Logged out successfully' }); }
});

/**
 * GET /api/auth/me
 * ✅ แก้ไข: เพิ่มการเช็คเพื่อแก้ตัวแดง และคง payment: true ไว้
 */
router.get('/me', authenticate as express.RequestHandler, async (req: Request, res: Response): Promise<void> => {
  try {
    const authReq = req as AuthRequest;

    // 1. ตรวจสอบความปลอดภัยเพื่อแก้ตัวแดง
    if (!authReq.user) {
      res.status(401).json({ success: false, error: 'Unauthorized' });
      return;
    }

    // 2. ดึงข้อมูลล่าสุดจาก DB โดยใช้ ID จาก Token
